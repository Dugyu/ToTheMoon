...
#region Members
  ...
  private void RunScript(List<double> s, List<Vector3d> v, int ry, int rx, double sizeX, double sizeY, Point3d focus, ref object A)
  {
    ...
    double max = 0.0;
    //looping the elevation list, find the maxHeight 
    ...
    //looping through all possible index pairs in the grid ry x rx
    for (int j = 0; j < ry; ++j) {
      for(int i = 0; i < rx; ++i) {
    ...
    // GENERATE PATH to visitor center
    //Start
    int startJ = minJ;
    int startI = minI;
    ...
    //GENERATE THE LARGE CROSS at the highest point on the mountain
